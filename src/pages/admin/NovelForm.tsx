...
export default function NovelForm() {
  ...
  const handleDeleteChapter = async () => {
    ...
    try {
      ...
      setSelectedChapters(prev => prev.filter(id => id !== deleteChapterId));
  ...
  const handleSelectChapter = (chapterId: string, checked: boolean) => {
    if (checked) {
      ...
      setSelectedChapters((prev) => prev.filter((cId) => cId !== chapterId));
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedChapters.length; i += LOG_BATCH_SIZE) {
        ...
        await Promise.all(
          batch.map(chapterId => logAdminAction("DELETE", "CHAPTER", chapterId, { novel_id: id }))
        );
      }
      ...
      toast({
        title: "Pemberitahuan",
        description: `Hanya ${deletedCount} chapter yang berhasil dihapus karena interupsi. Silakan ulangi sisanya.`,
        variant: "destructive",
      });
    } finally {
      setSelectedChapters([]);
